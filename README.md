...
- `STARTUP_CONCURRENCY_TEST=true` (default) runs a throughput sweep after the self-test, sending the same prompt at concurrency levels 2 through `min(8, PROMPT_MAX_PARALLEL)` via the foreground lane. This blocks startup and prints a table of wall-clock time and tokens/sec per level. Set to `false` to skip.
